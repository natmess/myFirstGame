            ...
            if hasFinished {
                ...
                scene.currentScore.run(SKAction.scale(to: 0, duration: 0.4)) {
                    ...
                    scene.gameBG.run(SKAction.scale(to: 0, duration: 0.4)) {
                        ...
                        self.scene.gameLogo.run(SKAction.move(to: CGPoint(x: 200, y: (self.scene.frame.size.height / 2)), duration: 0.5)) {
                            self.scene.playButton.isHidden = false
                            self.scene.playButton.run(SKAction.scale(to: 1, duration: 0.3))
                            self.scene.bestScore.run(SKAction.move(to: CGPoint(x: 200, y: self.scene.gameLogo.position.y - 50), duration: 0.3))
                        }
                }
            }
        }
